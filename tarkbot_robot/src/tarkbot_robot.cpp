...
RMRobot::RMRobot()
{
    ...
    init_vel_data[5] = 0;
    //发送串口数据
    sendSerialPacket(init_vel_data, 6, ID_CHASSIS_vel);
...
void RMRobot::recvDataHandle(uint8_t* buffer_data)
{
    ...
    if(buffer_data[3] == ID_CPR2ROS_DATA)
    {
        ...
        //解析机器人速度
        vel_data_.linear_x = ((double)((int16_t)(buffer_data[4]*256+buffer_data[5]))/1000);
        vel_data_.linear_y = ((double)((int16_t)(buffer_data[6]*256+buffer_data[7]))/1000);
        vel_data_.angular_z = ((double)((int16_t)(buffer_data[8]*256+buffer_data[9]))/1000);
